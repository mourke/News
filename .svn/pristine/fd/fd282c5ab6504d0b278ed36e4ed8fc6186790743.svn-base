...
public class SearchBar extends Widget {
  ...
  SearchBar(Rect frame, SearchBarDelegate delegate) {
    ...
    textLabel = new Label("", new Rect(frame.x + TEXT_INSET, frame.y, 0, 22));
    placeholderTextLabel = new Label(PLACEHOLDER_TEXT, new Rect(textLabel.frame.x, textLabel.frame.y, textLabel.frame.width, textLabel.frame.height));
    textLabel.setFont(captionFont);
    textLabel.setMaxWidth(frame.width - (TEXT_INSET * 2));
    placeholderTextLabel.setFont(captionFont);
    placeholderTextLabel.setTextColor(color(0, 0, 0, 102));
    caretFrame = new Rect(textLabel.frame.x, frame.y, 1, 22);
  ...
  @Override void draw() {
    ...
    if (focused && pulseCounter >= PULSE_COUNTER_MAX/2) {
      fill(color(0));
      rect(caretFrame.x, caretFrame.y, caretFrame.width, caretFrame.height);
    }
  ...
  private void updateCaretPosition() {
    caretFrame.x = textLabel.frame.x + textLabel.frame.width;
  }
