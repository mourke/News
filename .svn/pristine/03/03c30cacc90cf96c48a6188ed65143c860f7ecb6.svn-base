...
class SegmentedWidget extends Widget implements ButtonDelegate {
  ...
  void addSegment(String text) {
    ...
    if (segments.size() != 0) {
      ...
      segment = new SegmentedButton(new Rect(previousFrame.origin.x + previousFrame.size.width + SPACING, frame.origin.y, 0, frame.size.height), text, this);
    } else {
      segment = new SegmentedButton(new Rect(frame.origin.x, frame.origin.y, 0, frame.size.height), text, this);
      segment.setChecked(true);
    }
    
    frame.size.width += SPACING + segment.frame.size.width; 
